...
void btns(idata *cuda_inpoup,int N,int num_streams) 
{
    ...
    for(cbs=N; cbs<=N; cbs=cbs<<1) 
    {
        for(cd=cbs>>1; cd>0; cd=cd>>1)
        {
            bitonic_kernel<<<num_of_blocks,num_of_threads,0,stream>>>(cuda_inpoup,cd,cbs);
        }
    }
}
...
int main(int argc, char *argv[]) {
    ...
    aptr = cuda_inpoup;
    oets(aptr,N_per_stream,num_streams);
    ...
    for(int i=0;i<num_batches/2;i++) {
    	...
    	for(int j=0;j<num_batches/2;j++) {   //even round
    		...
    		btns(aptr,N_per_stream,num_streams);
	}
    	cudaDeviceSynchronize();
